			...
			<template v-if="commentData.list.length > 0">
				<div class="comment-detail" v-for="(item, index) in commentData.list" :key="index">
					...
					<div class="comment-body">
						<p>{{ item.content }}</p>
		        <div class="rater">
		        	<rater v-model="item.rater" :margin="raterConfig.margin" slot="value" :max="raterConfig.max" :font-size="raterConfig.fontSize" :active-color="raterConfig.activeColor" disabled></rater>
        ...
        <div class="comment-popup-body">
        	<group>
          	<x-textarea v-model="textareaData"  placeholder="请输入评论"></x-textarea>
			      <cell title="好评">
		        	<rater v-model="rater" :margin="raterConfig.margin" slot="value" :max="raterConfig.max" :font-size="raterConfig.fontSize" :active-color="raterConfig.activeColor"></rater>
