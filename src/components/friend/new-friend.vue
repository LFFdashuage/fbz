<!-- 
	我的好友
 -->

<template>
	<div class="friend">
		<group>
      <div class="add-btn">报名人员
				<div @click="addFriend" class="add-btn-click fa fa-plus"></div>
			</div>
			<swipeout>
				<swipeout-item v-show="item.value" @on-close="swipeoutClose(item, index)" @on-open="swipeoutOpen(item, index)" :ref="'swipeout' + index" v-for="(item, index) in friendData" :key="index" transition-mode="reveal">
	        <div slot="right-menu">
	          <swipeout-button @click.native="deleteFriend(index+1,item)" type="warn">删除</swipeout-button>
	        </div>
	        <div slot="content" class="user-block">
						
						<div class="user-block-info">
              <div>
                <span>{{ item.name }}</span>
              </div>
							<div>
                {{ item.idcard }} 
              </div>
						</div>
            <div @click="friendBtn(item, index)" class="friend-btn-dele fa fa-plus"></div>
					</div>
	      </swipeout-item>
			</swipeout>
		</group>

		<div v-transfer-dom>
			<popup v-model="friendPopup.status" @on-hide="hidePopup" is-transparent>
				<div class="friend-popup">
					<div class="friend-popup-btn">
						<!-- <div class="friend-popup-cancel">取消</div> -->
						<div class="friend-popup-sure" @click="friendPopupSure">确定</div>
					</div>

					<scroller lock-x :height="scrollerInfo.height + 'px'" @on-scroll-bottom="loadMore" ref="scrollerBottom" :scroll-bottom-offst="200">
						<div class="friend-btn-add">
							<x-button @click.native="friendAdd" mini>新增</x-button>
						</div>
		        <div class="friend-block" v-for="(item, index) in friendData" :key="index">
							<div @click="popuopFriendBtn(item)" class="friend-btn fa fa-edit"></div>
							<div class="friend-block-info">
								<p>
									<span>{{ item.name }}</span>
									<label>{{ item.tel }}</label>
								</p>
								<p>{{ item.idcard }}</p>
							</div>
							
							<check-icon class="friend-check" :value.sync="item.value"></check-icon>
						</div>
					</scroller>
				</div>
      </popup>
		</div>
	</div>
</template>

<script type="text/babel">
import { Scroller, Group, Cell, CheckIcon, Swipeout, SwipeoutItem, SwipeoutButton, XInput, XButton, FormPreview, Popup, TransferDomDirective as TransferDom } from "vux";

export default {
  name: "elNewFriend",
  directives: {
    TransferDom
  },
  components: {
    Scroller, Group, Cell, CheckIcon, Swipeout, SwipeoutItem, SwipeoutButton, XInput, XButton, FormPreview, Popup
  },
  // props: ['value'],
  data() {
    return {
      title: "我的好友",
      scrollerInfo: {
        height: 300,
        offsetBottom: 160,
        onFetching: false,
        loadAll: false,
        pageNum: 1,
        pageSize: this.wordBook.pageSize,
        list: []
      },
      friendData: [],
      friend: [
        // {
        // 	value: false,
        // 	name: "戚瑞印1戚瑞印1戚瑞印1戚瑞印1戚瑞印1戚瑞印1戚瑞印1戚瑞印1戚瑞印1",
        // 	idcard: "372923199204054415",
        // 	tel: "15021417505",
        // 	showBtn: false
        // }
      ],
      friendPopupStatus: true,
      friendPopup: {
        status: false
      }
    };
  },
  mounted() {
    this.fetchData();
  },
  methods: {
    fetchData() {
      let _this = this;

      if (this.scrollerInfo.onFetching) return;
      this.scrollerInfo.onFetching = true;

      _this.$http
        .post(
          "/api/userInfo/friendList",
          _this.qs.stringify({
            customerId: _this.$store.state.user.userId,
            pageSize: _this.scrollerInfo.pageSize,
            pageNum: _this.scrollerInfo.pageNum
          })
        )
        .then(function(e) {
          if (e.data.code == 200) {
            let list = [];

            if (e.data.data.result && e.data.data.result.length > 0) {
              list = e.data.data.result.map(function(item, index) {
                return {
                  friendId: item.friendId,
                  idcard: item.idcard,
                  tel: item.mobile,
                  name: item.name,
                  value: false,
                  showBtn: false
                };
              });
            }

            if (list.length < _this.scrollerInfo.pageSize) {
              _this.scrollerInfo.loadAll = true;
            }

            if (_this.scrollerInfo.pageNum == 1) {
              _this.friendData = list;
            } else {
              _this.friendData = _this.friendData.concat(list);
            }

            _this.scrollerInfo.pageNum++;
          } else {
            _this.$vux.alert.show({
              content: e.data.msg
            });
          }
          _this.resetView();
        });
    },
    resetView() {
      let _this = this;
      this.$nextTick(() => {
        this.scrollerInfo.onFetching = false;
        this.$refs.scrollerBottom.reset();
      });
    },
    loadMore() {
      if (this.scrollerInfo.onFetching || this.scrollerInfo.loadAll) {
        this.$refs.scrollerBottom.reset();
      } else {
        this.fetchAgency();
      }
    },
    addFriend() {
      this.friendPopup.status = true;
    },
    deleteFriend(ind,obj) {
      obj.value = false;
      this.$emit("on-people-num", ind, obj);
    },
    friendBtn(obj, ind) {
      obj.showBtn = !obj.showBtn;
      if (obj.showBtn) {
        this.$refs["swipeout" + ind][0].open("right");
      } else {
        this.$refs["swipeout" + ind][0].close();
      }
    },
    popuopFriendBtn(obj) {
      this.$router.push({ name: "friendEdit", query: { code: obj.code } });
    },
    swipeoutClose(obj, ind) {
      this.friend.map(function(item) {
        item.showBtn = false;
      });
    },
    swipeoutOpen(obj, ind) {
      obj.showBtn = true;
    },
    friendPopupSure() {
      this.friendPopup.status = false;
    },
    friendAdd() {
      this.$router.push({ name: "friendAdd" });
    },
    hidePopup() {
      let list = [];

      if (this.friendData.length > 0) {
        list = this.friendData.filter(function(item, index) {
          return item.value;
        });
      }
      this.$emit("on-hide-popup", list);
    }
  }
};
</script>

<style lang="scss" scoped>
@import "~lib/sandal/core";
@import "~assets/css/core/functions",
  "~assets/css/core/mixins",
  "~assets/css/core/vars";

$addBtnW: 30px;
$addBorderColor: #f3f3f3;

$friendBtnW: 30px;
$friendCheckW: 23px;

.friend {
  .add-btn {
    position: relative;
    width: 100%;
    line-height: $inputH;
    padding: 0 $padding;
    background: #fff;
    @include halfpxline(0, $borderColor, 0, 0, 1px, 0);
    .add-btn-click {
      position: absolute;
      top: 65%;
      left: 20%;
      width: 17px;
      height: 17px;
      margin-top: - $addBtnW/2;
      line-height: 17px;
      text-align: center;
      border-radius: $addBtnW;
      border: $colorRedDeep solid 1px;
      // background: $colorYellowEasy;
      color: $colorRedDeep;
      font-size: 10px;

      &.delete {
        background: $colorRed;
      }
    }
  }

  .user-block {
    @include halfpxline(0, $borderColor, 0 , 0, 1px, 0);
    padding: 5px $padding + $friendCheckW 5px $padding;
    margin: 5px 10px;
    display: flex;
    border: $bgGray solid 1px;
    border-radius: 10px;

    &.active {
      .friend-btn-dele {
        transform: rotate(-90deg);
      }
    }
    // 删除按钮
    .friend-btn-dele {
      position: absolute;
      top: 30%;
      right: 5%;
      width: 17px;
      height: 17px;
      color: $colorRedDeep;
      border-radius: $addBtnW;
      border: $colorRedDeep solid 1px;
      transition: 300ms;
      display: flex;
      text-align: center;
      line-height: 17px;
      font-size: 10px;
      transform: rotate(45deg);

      &:before {
        margin: 0 auto;
      }
    }
    .user-block-info {
      display: flex;
      align-items: center;
      div {
        padding: $padding 5px;
      }
    }
  }
}

.friend-popup {
  padding-bottom: 40px;
  background: #fff;

  .friend-block {
    padding-top: $padding;
  }

  .friend-popup-btn {
    @extend %clearfix;
    padding: 0 $padding;
    line-height: $inputH;
    background: $fontColorBlack;
    color: #fff;

    & > div {
      float: left;
    }

    .friend-popup-sure {
      float: right;
    }
  }
  .friend-btn-add {
    width: 100%;
    padding: 0 $padding;
    line-height: $inputH;
    text-align: left;
  }

  .friend-block {
    @include halfpxline(0, $borderColor, 0 , 0, 1px, 0);
    padding: 5px $padding + $friendCheckW 5px $padding + $friendBtnW;
    display: flex;

    &.active {
      .friend-btn {
        transform: rotate(-90deg);
      }
    }
    .friend-btn {
      position: absolute;
      top: 0;
      left: 0;
      width: 40px;
      height: 100%;
      color: red;
      transition: 300ms;
      display: flex;
      align-items: center;
      text-align: center;

      &:before {
        margin: 0 auto;
      }
    }
    .friend-block-info {
      line-height: 1.5;
      color: $fontColorBlack;
      flex: 1;

      span {
        float: left;
        @include ellipsisMore(1);
        margin-right: 1em;
      }

      label {
        width: 11em;
        color: $fontColor;
        font-size: $fontSizeTips;
      }
    }
    .friend-check {
      position: absolute;
      top: 50%;
      right: $padding;
      margin-top: -$friendCheckW/2;
    }
  }
}
</style>