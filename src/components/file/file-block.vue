...
<template>
	<div class="file-block">
		...
		<div class="file-block-info">
			<span class="price">下载积分：{{ fileData.price }}</span>
			<span class="number">下载次数：{{ fileData.download }}</span>
		</div>
		<!-- <a ref="downloadFile" target="_blank" :href="downloadUrl" class="btn"></a> -->
		
		<!-- <div @click="download" :class="['download-btn', 'fa', 'fa-arrow-circle-down', {'disable': isDownload}]"></div> -->
		<div v-if="!isDownload" @click="download" class="download-btn">下载</div>
		<div v-else @click="download" :class="['download-btn', {'disable': isDownload}]">已下载</div>
	</div>
</template>
...
.file-block {
  ...
  background: #fff url("~assets/img/icon/download-tips.png") no-repeat;
  background-size: 4%;
  ...
  .price {
    padding-left: 20px;
    background: #fff url("~assets/img/icon/download-integral.png") no-repeat;
    background-size: 15%;
    background-position: 4% 50%;
	}
	.number{
		padding-left: 20px;
    background: #fff url("~assets/img/icon/download-number.png") no-repeat;
    background-size: 15%;
    background-position: 4% 50%;
	}
}
