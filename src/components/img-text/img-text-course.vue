...
<template>
	<div class="img-text-course" @click="imgTextClick" v-cloak>
		<div class="img-text">
			...
			<div class="text">
				<h5 v-if="imgTextData.remark" class="title">{{ imgTextData.remark }}</h5>
				<p v-if="imgTextData.date" class="address fa fa-calendar">{{ imgTextData.date }}</p>
				<p v-if="imgTextData.address" class="address fa fa-map-marker">{{ imgTextData.address }}</p>
			</div>
		</div>

		<div class="img-text-info" v-if="imgTextData.originalPrice">
