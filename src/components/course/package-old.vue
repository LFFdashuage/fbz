		...
		<timeline>
			<timeline-item v-for="(item, index) in courseList" :key="index">
				<div class="course-recent">
					...
					<el-img-text v-for="(periodItem, periodIndex) in item.periodList" :img-text-data="periodItem" :key="periodIndex"></el-img-text>

					<div class="img-text-info" v-if="item.originalPrice">
						<span>原价：{{ item.originalPrice }}元</span>
						<p>活动价：{{ item.price }}元</p>
					</div>
				</div>
			</timeline-item>
		</timeline>
	...
	export default {
		...
		methods: {
							...
							if(e.data.data && e.data.data.result && e.data.data.result.length > 0) {
								list = e.data.data.result.map(function(item, ind){
				  				...
				  				if(item.periods.length > 0) {
				  					periodList = item.periods.map(function(it, ind){
				  						let date = it.startDate + " - " + it.endDate;
				  						return {
												id: it.periodsId,
												remark: it.name,
												// remark: it.remark,
          ...
          this.$refs.scrollerBottom.reset();
			...
			goDetail (id) {
				// this.$router.push({ name: 'courseDetail', query: { id: id, type: 'course' } })
			}
		}
	}
</script>
