...
<template>
	<div class="img-text-relate" @click="goPage">
		<div class="img">
			<img :src="imgTextData.img" alt="图片">
    ...
    <div class="info">
      ...
      <p class="time">{{ imgTextData.startDate + " - " + imgTextData.endDate}}</p>
			<p class="info-footer">
        <span class="price">￥{{ imgTextData.originalPrice || imgTextData.price  | numToCash}}</span>
        <span class="original-price"> ￥{{ imgTextData.price | numToCash }}</span>
        <span class="address">{{ imgTextData.address }}杭州</span>
      </p>
    </div>
	</div>
</template>
...
export default {
  ...
  watch: {
    ...
    hour(newValue, oldValue) {
      if (newValue == 23) this.day = this.day - 1;
    },
    min(newValue, oldValue) {
      if (newValue == 59) this.hour = this.hour - 1;
    },
    second(newValue, oldValue) {
      if (newValue == 59) this.min = this.min - 1;
    }
  },
  mounted() {
    let _this = this;
  ...
  methods: {
    ...
    goPage() {
      ...
      console.log(this.imgTextData.url);
      if (this.imgTextData.url == "activityDetail") {
        this.$router.push({
          name: "activityDetail",
          query: { activityId: this.imgTextData.id }
        });
      } else {
        this.$router.push({
          name: "courseDetail",
          query: { periodsId: this.imgTextData.id }
        });
      }
    }
  }
...
.info {
  ...
  .info-footer {
    ...
    .address {
      ...
      color: $fontColorGray;
      float: right;
      background: url(~assets/img/icon/address.png) no-repeat ;
