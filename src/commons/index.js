...
Vue.prototype.goPage = (url) => {
    if(url && (url.name || url.path)) router.push(url);
}

Vue.prototype.isOwnEmpty = (obj) => {
    for (let name in obj) {
...
Vue.prototype.getUserInfo = (openId, url = "") => {
    Vue.http.post('/api/userInfo/getUserInfo',
        ...
        if (e.data.code == 200) {
            ...
            store.commit("updateUserAuthorIds", { authorIds: res.authorIds ? res.authorIds.split(',') : '' });
            store.commit("updateUserIsAvailable", { isAvailable: res.isAvailable });
            store.commit("updateUserExpiryTime", { expiryTime: res.expiryTime ? res.expiryTime.substr(0, 10) : "" });
...
// 播放次数增加
Vue.prototype.addPlayCount = (pid) => {
    // http://api地址:api端口/
    Vue.http.post('/api/product/playCount',
        qs.stringify({
            openId: store.state.user.openId,
            customerId: store.state.user.userId,
            productId: pid
