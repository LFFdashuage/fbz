...
export default {
  name: "address-add",
  components: {
    Group, Cell, Selector, CheckIcon, XInput, XButton, XAddress,
    ChinaAddressV3Data
  },
  data() {
    return {
  ...
  watch: {
    province() {
      this.addressValue = this.addressValue.concat(this.province.toString(), this.city.toString(), this.county.toString());
  ...
  methods: {
    ...
    fetchData() {
      ...
      _this.$http
        .post(
          "/api/userInfo/getCustomerAddressInfo",
          _this.qs.stringify({
        ...
        .then(function(e) {
          if (e.data.code == 200) {
            ...
            _this.province = responseData.provinceId;
            _this.city = responseData.cityId;
            _this.county = responseData.areaId;
