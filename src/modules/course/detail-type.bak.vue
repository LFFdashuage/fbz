	...
	<div class="detail" v-if="list.title" v-cloak>
      ...
      <template v-if="courseInfo.type == 'video' || courseInfo.type == 'course'">        		
	    ...
	    <template v-if="courseInfo.type == 'audio'">        		
				<el-audio @audio-btn-change="audioBtnChange" :audio-data="course" :course-info="list" :has-buy="hasBuy"></el-audio>
	    </template>
...
.detail-btn-bottom {
  ...
  .collection-icon {
    background: #fff url(~assets/img/icon/redHeart.png) no-repeat;
    background-position: 13px center;
    background-size: auto 40%;
  }

  .collection-icon-tow {
    background: #fff url(~assets/img/icon/heart.png) no-repeat;
    background-position: 13px center;
    background-size: auto 40%;
  }
