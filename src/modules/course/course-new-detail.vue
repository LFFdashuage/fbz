...
<template>
    ...
    <!-- 底部按钮 -->
        ...
        <div class="detail-bottom-btn buy" @click="goPage('curriculumOrder', { id: courseInfo.periodsId, type: 'course' })">马上订票</div>
			</div>
		</div>
	</div>
</template>
...
<script type="text/babel">
  import { TransferDom, Divider, XButton, Flexbox, FlexboxItem, Tab, TabItem, Swiper, SwiperItem, Sticky} from "vux";
  ...
  export default {
    ...
    methods: {
      ...
      collection() {
        let _this = this;
        _this.$http.post("/api/customer/collection",
            _this.qs.stringify({
              customerId: _this.$store.state.user.userId,
              goodsType: 4,
