		...
		<div class="container">
		    ...
		    <swiper v-model="tabSelected" height="100%" :show-dots="false" :threshold="tabChangeW">
		      <swiper-item v-for="(tabItem, ind) in tabData" :key="ind">
		    		...
		    		<template v-if="tabItem.value == 'package'">
							<el-course-package></el-course-package>
		    		</template>
		      </swiper-item>
		    </swiper>
			</div>
...
<script type="text/babel">
	...
	import elCoursePackage from 'components/course/package'

	export default {
		name: 'courseTypeDetail',
		components: { XTable, Card, Timeline, TimelineItem, Tab, TabItem, Swiper, SwiperItem, elCard, elImgText, elCourseTuijian, elCourseRecent, elCoursePackage, elCourseActivity },
		data () {
			return {
				title: '课程列表',
