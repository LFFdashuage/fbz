...
<template>
    ...
    <swiper v-model="tabSelected" height="100%" :show-dots="false" :threshold="tabChangeW">
      <swiper-item v-for="(item, index) in tabData" :key="index">
  			<el-card-order-package @on-load-more="loadMore" @on-data-change="dataChange" :card-data="item.list" :card-count="count" :card-index="index" :on-fetching="item.onFetching" :on-fetch-all="item.loadAll"></el-card-order-package>
      </swiper-item>
    </swiper>
	</div>
</template>
...
<script type="type/babel">
	...
	import elCardOrderPackage from 'components/card/card-order'

	export default {
		name: 'orderList',
		components: { Tab, TabItem, Swiper, SwiperItem, elCardOrderPackage },
		data () {
			return {
				title: '订单列表页面',
		...
		methods: {
			fetchData (obj, ind = 0) {
				let _this = this;
				_this.$http.post('/api/customer/package/order',
					_this.qs.stringify({
