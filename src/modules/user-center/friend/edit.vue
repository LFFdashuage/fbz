...
<script type="text/babel">
	...
	export default {
		...
		methods: {
			fetchData () {
				let _this = this;
				this.$http.post('/api/userInfo/getFriendInfo',
					this.qs.stringify({
	  				friendId: _this.$route.query.friendId
	  			})).then(function(e) {
						if(e.data.code == 200) {
							_this.name = e.data.data.name;
							_this.idcard = e.data.data.idcard;
							_this.mobile = e.data.data.mobile;
						} else {
